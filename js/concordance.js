...
export function build() {
  ...
  INDEX = new Map();
  for (const [texts, label] of [[B1_TEXTS, 'L'], [B2_TEXTS, 'L']]) {
    texts.forEach(t => t.parts.forEach(p => p.lines.forEach(ln =>
      add(INDEX, ln.zh, ln.es, ln.en, { type: 'dialog', lesson: t.l, title: t.t })
    )));
  }
  for (const reads of [B1_READINGS, B2_READINGS]) {
    reads.forEach(t => t.lines.forEach(ln =>
      add(INDEX, ln.zh, ln.es, ln.en, { type: 'reading', lesson: t.l, title: t.t })
    ));
  }
  return INDEX;
}
