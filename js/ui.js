...
export function setView(html) { const v = $('#view'); v.innerHTML = html; v.scrollTop = 0; return v; }

// Conjunto de tarjetas activas según ajustes
export function cardPool(ALL) {
  return ALL.filter(e => e.l <= settings.maxLesson && (settings.includeSup || !e.sup));
}
...
export function renderTokens(zh, mode) {
  ...
  for (const tok of segment(zh)) {
    if (tok.plain) {
      ...
      ruby.className = 'ruby'; ruby.innerHTML = '&nbsp;'; ruby.style.visibility = 'hidden';
    ...
    Array.from(tok.t).forEach((ch, i) => {
      ...
      else if (mode === 'tones') { ruby.textContent = syls[i] ? TONE_MARK[toneOf(syls[i])] : ' '; ruby.classList.add('tones-only'); }
  ...
  tok.entries.forEach((e, idx) => {
    ...
    if (uses.length) {
      ...
      uses.forEach(u => {
        ...
        spk.addEventListener('click', ev => { ev.stopPropagation(); speak(u.zh); });
        zh.appendChild(spk);
        const tr = document.createElement('div'); tr.className = 'use-tr'; tr.textContent = settings.lang === 'en' ? u.en : u.es;
        row.appendChild(zh); row.appendChild(tr);
        box.appendChild(row);
      });
      div.appendChild(box);
    }
...
export function popupEntry(hanzi) { showPopup({ t: hanzi, entries: DICT.get(hanzi) }); }
...
export function startQuiz(word) {
  ...
  function renderChar(idx) {
    ...
    writer.quiz({
      onComplete: () => {
        if (idx + 1 < chars.length) { status.textContent = '¡Bien! Siguiente...'; setTimeout(() => renderChar(idx + 1), 800); }
        else { status.textContent = '¡Excelente! Terminado 🎉'; setTimeout(() => modal.classList.add('hidden'), 1500); }
      }
    });
  }
  renderChar(0);
