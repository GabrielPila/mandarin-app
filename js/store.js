...
let state = {};
try { state = JSON.parse(localStorage.getItem(SRS_KEY) || '{}'); } catch (e) { state = {}; }

export function getState() { return state; }
export function getCard(id) { return state[id]; }
export function putCard(id, card) { state[id] = card; saveState(); }
export function saveState() { localStorage.setItem(SRS_KEY, JSON.stringify(state)); }
export function resetAll() { state = {}; saveState(); }

export const settings = {
  lang: 'es', includeSup: false, newPerDay: 10, maxLesson: 20,
  textSize: 'medium', theme: 'system', history: {}, voiceSpeed: 1.0, voiceURI: '',
  reverseCards: false, pairsBest: {}
};
