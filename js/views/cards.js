...
import { $, setView, cardPool, renderTokens, startQuiz, popupEntry } from '../ui.js';
...
export function startSRS() {
  ...
  runCards(queue.map(id => ({ id })), true);
}

export function startCram(cards) { runCards(shuffle(cards.map(e => ({ id: e.id }))), false); }

// items: [{id}]  (id puede ser normal o 'rev:<id>')
export function runCards(items, srsMode) {
  const total = items.length, startTime = Date.now();
  let idx = 0, firstCorrect = 0, answered = 0;
  ...
  function finish() {
    ...
    if (failedList.length) {
      ...
      $('#retry-failed').addEventListener('click', () => runCards(shuffle(failedList.map(e => ({ id: e.id }))), false));
    }
  ...
  function show() {
    ...
    v.querySelectorAll('.card-spk').forEach(b => b.addEventListener('click', ev => { ev.stopPropagation(); speak(e.h); }));
    const quizBtn = v.querySelector('.card-quiz');
    if (quizBtn) quizBtn.addEventListener('click', ev => { ev.stopPropagation(); startQuiz(e.h); });
    ...
    $('#card').addEventListener('click', () => {
      ...
      if (window.HanziWriter && hwCont && hwCont.innerHTML === '') {
        for (const ch of e.h) {
          ...
          const box = document.createElement('div'); box.className = 'hw-box'; hwCont.appendChild(box);
    ...
    $('#grade').querySelectorAll('button').forEach(b => {
      b.addEventListener('click', () => {
        const g = +b.dataset.g;
        answered++; gradeCounts[g]++;
        if (g >= 4) firstCorrect++; else failed.add(item.id);
