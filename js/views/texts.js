...
function renderReader(t, kind) {
  ...
  player.onStateChange(playing => { const b = $('#tg-audio'); if (b) b.textContent = playing ? '⏹️' : '🔊'; });
  $('#tg-audio').addEventListener('click', () => { if (player.isPlaying()) player.stop(); else player.play(0); });
  const update = () => {
