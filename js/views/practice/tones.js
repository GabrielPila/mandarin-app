...
export function renderTones() {
	...
	function round() {
		...
		const optionList = sample(
			[...opts].map((s) => s.split("").map(Number)),
			opts.size,
		);
      ...
      <button id="back" class="back-btn">← ${T("back")}</button>`);
		...
		optionList.forEach((o) => {
			...
			b.className = "cloze-opt tone-opt";
			b.textContent = patternLabel(o);
			b.addEventListener("click", () => {
				const ok = o.join("") === pat.join("");
				if (ok) correct++;
				optsEl.querySelectorAll("button").forEach((x) => {
					x.disabled = true;
					if (x.textContent === patternLabel(pat))
						x.classList.add("opt-correct");
					else if (x === b) x.classList.add("opt-wrong");
				});
				const rev = document.createElement("div");
				rev.className = "tone-reveal";
