...
export function renderPairs() {
  ...
  let mode = 'mean';
  $('#m-mean').addEventListener('click', () => { mode = 'mean'; $('#m-mean').classList.add('on'); $('#m-pin').classList.remove('on'); });
  $('#m-pin').addEventListener('click', () => { mode = 'pin'; $('#m-pin').classList.add('on'); $('#m-mean').classList.remove('on'); });
  const grid = $('#lgrid');
...
function play(cards, mode, lesson) {
  let queue = sample(cards, cards.length), matched = 0, start = Date.now();
  ...
  function round() {
    if (!queue.length) {
      ...
      if (!prev || secs < prev) { settings.pairsBest[key] = secs; saveSettings(); }
      recordActivity();
    ...
    left.forEach(e => {
      const b = document.createElement('button'); b.className = 'pair-tile'; b.textContent = e.h; b.dataset.id = e.id;
      b.addEventListener('click', () => choose(b, e, 'l'));
      $('#col-l').appendChild(b);
    });
    right.forEach(e => {
      const b = document.createElement('button'); b.className = 'pair-tile'; b.textContent = rightText(e); b.dataset.id = e.id;
      b.addEventListener('click', () => choose(b, e, 'r'));
      $('#col-r').appendChild(b);
    });
    function choose(btn, e, side) {
      if (btn.classList.contains('gone')) return;
      if (!sel) { sel = { btn, e, side }; btn.classList.add('sel'); return; }
      if (sel.side === side) { sel.btn.classList.remove('sel'); sel = { btn, e, side }; btn.classList.add('sel'); return; }
      if (sel.e.id === e.id) {
        sel.btn.classList.add('gone'); btn.classList.add('gone'); sel.btn.classList.remove('sel');
        sel = null; done++; matched++;
        if (done === group.length) setTimeout(round, 400);
      } else {
        const bad = sel.btn; btn.classList.add('bad'); bad.classList.add('bad');
        setTimeout(() => { btn.classList.remove('bad'); bad.classList.remove('bad', 'sel'); }, 500);
        sel = null;
      }
    }
    $('#back').addEventListener('click', () => nav('study'));
