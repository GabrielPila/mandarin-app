...
function candidates(line) {
  return wordTokens(line.zh).filter(t => t.e && !SKIP_POS.test(t.e.pos || '') && t.e.id);
}
...
export function renderCloze() {
  ...
  function round() {
    ...
    const sameLes = ALL.filter(e => e.l === target.e.l && e.h !== target.h && e.pos === target.e.pos);
    const samePos = ALL.filter(e => e.pos === target.e.pos && e.h !== target.h);
    let distr = sample(sameLes.length >= 3 ? sameLes : samePos, 3);
    if (distr.length < 3) distr = distr.concat(sample(ALL.filter(e => e.h !== target.h), 3 - distr.length));
    ...
    opts.forEach(o => {
      ...
      b.className = 'cloze-opt'; b.textContent = o.h;
      b.addEventListener('click', () => {
        ...
        sent.innerHTML = ''; sent.appendChild(renderTokens(line.zh, 'pinyin'));
