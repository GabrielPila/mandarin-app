...
import { numToHanzi, priceToHanzi, timeToHanzi, dateToHanzi } from '../../numbers.js';
...
function makeItem(cat) {
  ...
  if (c === 'num') { const n = rnd(0, 9999); return { hanzi: numToHanzi(n), answer: String(n), ph: '1234' }; }
  if (c === 'price') { const k = rnd(1, 999), m = rnd(0, 9); return { hanzi: priceToHanzi(k, m), answer: m ? `${k}.${m}` : String(k), ph: '¥12.5' }; }
  if (c === 'time') { const h = rnd(1, 12), m = pick([0, 5, 10, 15, 30, 45]); return { hanzi: timeToHanzi(h, m), answer: `${h}:${String(m).padStart(2, '0')}`, ph: '8:30' }; }
  const mo = rnd(1, 12), d = rnd(1, 28); return { hanzi: dateToHanzi(mo, d), answer: `${mo}/${d}`, ph: '10/25' };
}
...
export function renderNumbers() {
  ...
  let cat = 'num';
  $('#cat').querySelectorAll('button').forEach(b => b.addEventListener('click', () => {
    cat = b.dataset.c; $('#cat').querySelectorAll('button').forEach(x => x.classList.remove('on')); b.classList.add('on');
  }));
...
function play(cat) {
  ...
  function round() {
    ...
    $('#ans').focus();
    const submit = () => {
      const got = $('#ans').value.trim().replace(/[¥$\s]/g, '');
      const want = it.answer.replace(/[¥$\s]/g, '');
      const ok = got === want;
      if (ok) correct++;
      $('#result').innerHTML = `<span class="${ok ? 'opt-correct' : 'opt-wrong'}">${ok ? T('correct') : T('incorrect')}</span>
