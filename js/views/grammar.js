...
export function renderGrammar() {
  ...
  grid.querySelectorAll('button').forEach(b => b.addEventListener('click', () => {
    ...
    b.classList.add('on'); currentTag = b.dataset.tag; draw();
  }));

  function draw() {
    const list = $('#glist'); list.innerHTML = '';
    const items = GRAMMAR.filter(g => currentTag === 'all' ||
    ...
    list.querySelectorAll('.g-ex').forEach(el => el.addEventListener('click', () => {
