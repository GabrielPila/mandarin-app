...
test("syllables: one syllable per character", () => {
	assert.deepEqual(syllables({ h: "图书馆", p: "tú shū guǎn" }), [
		"tú",
		"shū",
		"guǎn",
	]);
...
test("syllables: erhua gets empty ruby on 儿", () => {
	assert.deepEqual(syllables({ h: "面条儿", p: "miàn tiáor" }), [
		"miàn",
		"tiáor",
		"",
	]);
	assert.deepEqual(syllables({ h: "一会儿", p: "yí huìr" }), [
		"yí",
		"huìr",
		"",
	]);
...
test("syllables: apostrophe and hyphen separate syllables", () => {
	...
	assert.deepEqual(syllables({ h: "第一次", p: "dì-yī cì" }), [
		"dì",
		"yī",
		"cì",
	]);
